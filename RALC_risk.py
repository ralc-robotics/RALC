...
class MPC:
	def __init__(self, path, with_obs, L, Ts, N, xmax, xmin, umax, umin, Q_w, Qf_w, R_w, method = None):
		...
		try:
			self.solver = forcespro.nlp.Solver.from_directory("./RALC_risk")
		except:
			print('Creating MPC')
			self.create_MPC()
...
#		x0 = np.zeros((self.nx + self.nx*self.nx + self.nu, self.N))
	...
	def inequality(self, z,p):
		...
		return [mean + theta*np.sqrt(1+beta**2) - delta + beta*Sigma]
	...
	def create_MPC(self):
		...
		codeoptions = forcespro.CodeOptions('RALC_risk')
