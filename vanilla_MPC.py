...
class MPC:
	...
	def solve_MPC(self, x_mean, obs, goal, obs_box_, car_box_, extent_o, extent_r, t):
		...
		tiled = np.tile(np.concatenate([self.Q_w, self.R_w, self.Qf_w, obs[:2], [r_o + r_r]]),(self.N,1))
	...
	def create_MPC(self):
		...
		def inequality(z,p):
			x_o = p[3*self.nx + self.nu:3*self.nx + self.nu+self.ny]
			r = p[3*self.nx + self.nu+self.ny]
			x = z[2*self.nu:2*self.nu+self.ny]
			return [r**2 - (x_o - x).T @ (x_o - x)]
