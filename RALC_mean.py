...
class MPC:
	def __init__(self, path, with_obs, L, Ts, N, xmax, xmin, umax, umin, Q_w, Qf_w, R_w, method = None):
		...
		self.npar = self.nx + 2*self.nx + self.nu + self.max_data*self.max_data*self.nx + self.max_data*self.nx + self.nx + self.nx + self.max_data*(self.nx+self.nu) + 1 + 3 + self.ny + self.ny*self.ny
					#goal   + Q, Qf     + R       + L                                   + alpha                 + var     + length  + X_train               +  rad + beta,theta,alpha + obs_mean + obs_cov12
		...
		try:
			self.solver = forcespro.nlp.Solver.from_directory("./RALC_mean")
		except:
			print('Creating MPC')
			self.create_MPC()
		self.solver_backup = forcespro.nlp.Solver.from_directory("./nominal_MPC")
...
#		x0 = np.zeros((self.nx + self.nx*self.nx + self.nu, self.N))
	...
	def update_dist_np(self, X_s_bar, Q, w_m, w_c):
		...
		return mu, np.linalg.chol(Sigma + Q + 1e-5*np.eye(n)).T
	...
	def create_MPC(self):
		...
		codeoptions = forcespro.CodeOptions('RALC_mean')
		...
		self.solver = model.generate_solver(options=codeoptions)
	...
	def render(self, world):
		np.save(self.path + 'waypoints.npy', world.waypoints_array)
		T = len(self.X_mean)
		...
		def func(t):
			...
			plt.savefig(self.path + 'snapshot_{}.eps'.format(t), format='eps')
