...
class World(object):
    def __init__(self, Ts, carla_world, hud, args):
        ...
        self.save_goals = args.save_goals
        self.load_goals = args.load_goals
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            spawn_point = spawn_points[148] if spawn_points else carla.Transform()
            self.player = self.world.try_spawn_actor(blueprint, spawn_point)
            self.show_vehicle_telemetry = False
            self.modify_vehicle_physics(self.player)
        ...
        if self.with_obs:
            while self.obstacle is None:
                ...
                spawn_point_obs = spawn_points_obs[74] if spawn_points_obs else carla.Transform()
                self.obstacle = self.world.try_spawn_actor(blueprint_obs, spawn_point_obs)
                self.modify_vehicle_physics(self.obstacle)
        ...
        self.camera_manager = CameraManager(self.path, self.player, self.hud, self._gamma)
        ...
        if self.load_goals:
            ...
            waypoint = self.map.get_waypoint(spawn_point.location, project_to_road = True)
            self.waypoints = waypoint.next(self.waypoint_sep)[0]
        if self.sync:
            self.world.tick()
        else:
    ...
    def tick(self, clock):
        ...
        if self.load_goals:
            ...
            waypoint = self.map.get_waypoint(self.player.get_location(), project_to_road = True)
            self.waypoints = waypoint.next(self.waypoint_sep)[0]
...
class MPCControl(object):
    def __init__(self, tm, world, Ts, gp, method):
        self.Ts = Ts
        if gp:
            if method=='mean':
                from RALC_mean import MPC
            else:
                from RALC_risk import MPC #For GP-based
        else:
    ...
    def parse_events(self, world, t):
        ...
        self.tm.vehicle_percentage_speed_difference(world.obstacle, 70)
...
class CameraManager(object):
    def __init__(self, path, parent_actor, hud, gamma_correction):
        self.path = path
        self.sensor = None
    ...
    def _parse_image(weak_self, image, mini_flag):
        ...
        if self.recording:
            image.save_to_disk(self.path + '/%08d' % image.frame)
...
def game_loop(args):
    ...
    try:
        ...
        controller = MPCControl(traffic_manager, world, Ts, args.gp, args.method)
